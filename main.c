...
void test_imu_kalman(void) {
    ...
    while (1) {
        // Lire toutes les donnees du capteur (accéléromètre, gyroscope et température)
        if (MPU6050_ReadAll(&mpu) == MPU6050_Result_Ok) {

            // Récuperer les valeurs des capteurs
			...
			// Fonction pour mapper les angles [-180,180] vers [0, 200] (longueur barre)
			int map_angle_to_bar(float angle) {
			    if (angle < -180) angle = -180;
			    if (angle > 180) angle = 180;
			    return (int)((angle + 180.0f) * (200.0f / 360.0f));
			}

			// Afficher texte + barre pour chaque angle
			char buffer[30];
			int bar_length;

			// ROLL
			sprintf(buffer, "ROLL : %.1f", roll);
			ILI9341_Puts(10, 190, buffer, &Font_7x10, ILI9341_COLOR_BLACK, ILI9341_COLOR_WHITE);


			// PITCH
			sprintf(buffer, "PITCH: %.1f", pitch);
			ILI9341_Puts(10, 210, buffer, &Font_7x10, ILI9341_COLOR_BLACK, ILI9341_COLOR_WHITE);


			// YAW
			sprintf(buffer, "YAW  : %.1f", yaw);
			ILI9341_Puts(10, 230, buffer, &Font_7x10, ILI9341_COLOR_BLACK, ILI9341_COLOR_WHITE);

			// Définir la zone centrale comme référence
			int centerX = 160;  // milieu horizontal (320/2)
			int centerY = 120;  // milieu vertical   (240/2)
			static int oldX = 160, oldY = 120;

			// Facteur d'échelle pour amplifier les mouvements
			float scale = 2.0f;

			// Calcul position du point en fonction du yaw (X) et du roll (Y)
			int pointX = centerX - (int)(yaw * scale);
			int pointY = centerY + (int)(roll * scale);

			// Limiter le point aux bords de l'écran
			if (pointX < 0) pointX = 0;
			if (pointX > 319) pointX = 319;
			if (pointY < 0) pointY = 0;
			if (pointY > 239) pointY = 239;

			// Effacer l'ancienne croix (remplacer par fond blanc)
			...
			// Dessiner une croix rouge à la nouvelle position
